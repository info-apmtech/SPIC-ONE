...
namespace Spic.Infrastructure.Services
{
    ...
    public class LiquidationCycleService : ILiquidationCycleService
    {
        ...
        {
            return rows
                // Top-5 is Dealer Performance only. Company/Warehouse rows are
                // not dealers and must never appear in Top-5 Dealer charts.
                .Where(x => !string.IsNullOrWhiteSpace(x.DealerName) &&
                            x.DealerName != "-" &&
                            !string.Equals(x.Source, CompanySource, StringComparison.OrdinalIgnoreCase) &&
                            !string.Equals(x.DealerType, "Warehouse", StringComparison.OrdinalIgnoreCase))
