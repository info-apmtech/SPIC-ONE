...
namespace SpicAPI.Controllers
{
    ...
    public class WarehouseController : GenericCrudController<Warehouse>
    {
        ...
        private IActionResult? ValidatePrimaryLocation(Warehouse entity)
        {
            ...
            if (!entity.DistrictId.HasValue || entity.DistrictId.Value <= 0)
    ...
    public class RackPointController : GenericCrudController<RackPoint>
    {
        ...
        public override async Task<IActionResult> Create([FromBody] RackPoint entity)
        {
            ...
            NormalizeInactiveLocation(entity);
            var locError = ValidatePrimaryLocation(entity);
            if (locError != null) return locError;
        ...
        private static void NormalizeInactiveLocation(RackPoint entity)
        {
            // Active flow remains exactly as before.
            if (entity.IsActive)
                return;

            // Inactive location is optional. Never send FK value 0 to PostgreSQL.
            if (!entity.StateId.HasValue || entity.StateId.Value <= 0)
                entity.StateId = null;
